...
function JoinCheck() {
    //1)아이디 유효성
    let m_id = $("#MainContent_mb_id").val();
    if (m_id == "") {
    ...
    //3)이름 유효성
    let mb_name = $("#MainContent_mb_name").val();
    let nameRegExp = /^[가-힣]{2,20}$/;
    if (mb_name.length == 0) {
        alert("이름을 입력해주세요");
    ...
    //4)이메일 유효성
    let email = $("#MainContent_email1").val();
    let domain = $("#MainContent_email2").val();
    let emailRegExp = /^[A-Za-z0-9_\.\-]{5,20}$/;
    let domainRegExp = /^[A-Za-z0-9\-\.]+\.[A-Za-z]{2,3}$/;
    if (email.length == 0) {
        alert("이메일을 입력해주세요");
    ...
    //5)휴대전화 유효성 (선택항목)
    let hp2 = $("#MainContent_hp2").val();
    let hp3 = $("#MainContent_hp3").val();
    let hpRegExp = /^[0-9]{3,4}$/;
...
function UpdateCheck() {
    ...
    let domain = $("#MainContent_email2").val();
    let emailRegExp = /^[A-Za-z0-9_\.\-]{5,20}$/;
    let domainRegExp = /^[A-Za-z0-9\-]+\.[A-Za-z0-9\-]+/;
    if (email.length == 0) {
        alert("이메일을 입력해주세요");
        $("#MainContent_email1").focus();
        return false;
